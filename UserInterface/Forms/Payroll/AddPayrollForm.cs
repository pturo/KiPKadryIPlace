...
{
    public partial class AddPayrollForm : BaseAddEditForm
    {
        #region Fields
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if(ValidateForm())
            {
                ...
                string query = "insert into dbo.Payrolls (EmployeeId, RateHour, HourDay, DaysWorked, DatePay, Month, Description) " +
                    "values (@EmployeeId, @RateHour, @HourDay, @DaysWorked, @DatePay, @Month, @Description);";
                ...
                if(result > 0)
                {
                    MessageBox.Show("Pomyślnie dodano listę płac do bazy!");
                    Close();
                }
                else
                {
                    MessageBox.Show("Wystąpił błąd podczas dodawania listy płac do bazy!");
                }
            }
        }
